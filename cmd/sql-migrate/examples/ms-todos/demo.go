...
func runAdd(ctx context.Context, db *sql.DB, args []string) error {
	...
	_, err := db.ExecContext(
		ctx,
		"INSERT INTO todos (id, title) VALUES (@p1, @p2)",
		id,
		title,
	)
	if err != nil {
...
func runDone(ctx context.Context, db *sql.DB, args []string) error {
	...
	result, err := db.ExecContext(
		ctx,
		"UPDATE todos SET status = 'done', completed_at = SYSDATETIME() WHERE id = @p1",
		args[0],
	)
	if err != nil {
...
func runSeed(ctx context.Context, db *sql.DB) error {
	...
	todos := []seedTodo{
		...
		{
			ID:    "seed0020",
			Title: "Deploy v2.3.1 to staging",
		},
		{
			ID:    "seed0021",
	...
	for _, t := range todos {
		_, err := db.ExecContext(
			ctx,
			`IF NOT EXISTS (SELECT 1 FROM todos WHERE id = @p1)
    INSERT INTO todos (id, title) VALUES (@p1, @p2)`,
			t.ID,
			t.Title,
		)
	...
	for _, id := range []string{"seed0010", "seed0011"} {
		_, err := db.ExecContext(
			ctx,
			"UPDATE todos SET status = 'done', completed_at = SYSDATETIME() WHERE id = @p1 AND status != 'done'",
			id,
		)
		if err != nil {
