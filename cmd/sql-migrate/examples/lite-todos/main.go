...
func main() {
	...
	fs := flag.NewFlagSet("lite-todos", flag.ContinueOnError)
	...
	liteURL := os.Getenv("LITE_URL")
	if liteURL == "" {
		liteURL = "todos.db"
	}

	db, err := sql.Open("sqlite", liteURL+"?_pragma=foreign_keys(1)")
	if err != nil {
...
func printVersion(w *os.File) {
	fmt.Fprintf(w, "lite-todos v%s\n", version)
}

func printUsage(w *os.File) {
	fmt.Fprintln(w, `USAGE
   lite-todos <command> [args]
...
ENVIRONMENT
   LITE_URL            SQLite database path (default: todos.db, loaded from .env)`)
}
