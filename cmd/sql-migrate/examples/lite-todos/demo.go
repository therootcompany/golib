...
func runList(ctx context.Context, db *sql.DB, args []string) error {
	...
	for rows.Next() {
		...
		if completedAt.Valid {
			displayStatus = fmt.Sprintf("done %s", completedAt.String[:10])
		}
		pri := ""
		if priority > 0 {
...
func runSeed(ctx context.Context, db *sql.DB) error {
	...
	for _, t := range todos {
		_, err := db.ExecContext(
			ctx,
			"INSERT OR IGNORE INTO todos (id, title) VALUES (?, ?)",
			t.ID,
			t.Title,
		)
		if err != nil {
	...
	for _, id := range []string{"seed0010", "seed0011"} {
		_, err := db.ExecContext(
			ctx,
			"UPDATE todos SET status = 'done', completed_at = datetime('now') WHERE id = ? AND status != 'done'",
			id,
		)
		if err != nil {
