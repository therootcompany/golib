...
const description = "Generate sample JSON values from a JSON document."
...
func main() {
	...
	fs := flag.NewFlagSet(os.Args[0], flag.ContinueOnError)
	fs.IntVar(&cfg.maxStr, "maxstr", 40, "max string length before truncation (0 = no limit)")
	...
	cfgs := jsontypes.SampleConfig{MaxStringLen: cfg.maxStr}
	for _, line := range jsontypes.SampleWithConfig(data, cfgs) {
		fmt.Println(line)
	}
}
