...
// AudienceClaims returns claims with the given audience.
func AudienceClaims(sub string, aud jwt.Audience) *jwt.IDTokenClaims {
	c := TestClaims(sub)
	c.Aud = aud
	return c
}

// CustomClaims embeds IDTokenClaims and adds extra fields for testing
