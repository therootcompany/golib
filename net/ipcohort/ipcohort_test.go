...
func TestContains_Mixed(t *testing.T) {
	...
	if mustContain(t, c, "1.2.3.5") {
		t.Error("false positive for host-adjacent")
	}
}

func TestContains_UnparseableReturnsError(t *testing.T) {
	c, _ := ipcohort.Parse([]string{"1.2.3.4"})
	found, err := c.Contains("not-an-ip")
	if err == nil {
		t.Fatal("expected error for unparseable IP")
	}
	if found {
		t.Error("expected found=false for unparseable IP")
	}
}
...
func TestLoadFile(t *testing.T) {
	...
	c, err := ipcohort.LoadFile(path, 0)
	if err != nil {
		t.Fatal(err)
	}
	if c.Size() != 3 {
...
func TestLoadFiles_Merge(t *testing.T) {
	...
	c, err := ipcohort.LoadFiles(0, f1, f2)
	if err != nil {
		t.Fatal(err)
	}
	if c.Size() != 3 {
...
func TestLoadFile_MaxBytes(t *testing.T) {
	dir := t.TempDir()

	// 200-byte file, 100-byte cap → ErrInputTooLarge (stat-time short-circuit).
	big := filepath.Join(dir, "big.txt")
	if err := os.WriteFile(big, []byte(strings.Repeat("1.2.3.4\n", 25)), 0o644); err != nil {
		t.Fatal(err)
	}
	if _, err := ipcohort.LoadFile(big, 100); !errors.Is(err, ipcohort.ErrInputTooLarge) {
		t.Errorf("LoadFile with maxBytes=100: got %v, want ErrInputTooLarge", err)
	}

	// Same file, generous cap → no error.
	if _, err := ipcohort.LoadFile(big, 1<<20); err != nil {
		t.Errorf("LoadFile with maxBytes=1MiB: %v", err)
	}

	// maxBytes=0 disables.
	if _, err := ipcohort.LoadFile(big, 0); err != nil {
		t.Errorf("LoadFile with maxBytes=0: %v", err)
	}
}

func TestParseCSV_MaxBytes(t *testing.T) {
	// ParseCSV doesn't have a stat seam, so the cap is enforced via
	// LimitReader + counting reader.
	body := strings.NewReader(strings.Repeat("1.2.3.4\n", 25)) // 200 bytes
	c, err := ipcohort.ParseCSV(body, 100)
	if !errors.Is(err, ipcohort.ErrInputTooLarge) {
		t.Errorf("ParseCSV with maxBytes=100: got %v, want ErrInputTooLarge", err)
	}
	if c == nil {
		t.Error("ParseCSV: expected partial cohort, got nil")
	}
}
