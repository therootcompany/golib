...
// TestGeoIPDBDownloadAuth verifies the first fetch issues a Basic-auth GET,
...
func TestGeoIPDBDownloadAuth(t *testing.T) {
	const (
		user = "123456"
		key  = "testkey"
	)
	...
	f, err := NewGeoIPDB(confPath)
	if err != nil {
		t.Fatalf("NewGeoIPDB: %v", err)
	}
	f.Dir = dir
	f.BaseURL = srv.URL
	f.HTTPClient = srv.Client()

	updated, err := f.Update(context.Background())
	if err != nil {
...
// TestGeoIPDBConditionalGET verifies the conditional-GET flow: a stale local
...
func TestGeoIPDBConditionalGET(t *testing.T) {
	...
	// Prime with v1.
	if updated, err := f.Update(context.Background()); err != nil || !updated {
...
// TestGeoIPDBDefaultFreshDays ensures a zero FreshDays defaults to a 3-day
// MaxAge gate.
func TestGeoIPDBDefaultFreshDays(t *testing.T) {
	dir := t.TempDir()
	confPath := writeConf(t, dir, "GeoLite2-City")
	f, err := NewGeoIPDB(confPath)
	if err != nil {
...
// TestGeoIPDBExplicitFreshDays checks a non-default FreshDays is applied.
func TestGeoIPDBExplicitFreshDays(t *testing.T) {
	dir := t.TempDir()
	confPath := writeConf(t, dir, "GeoLite2-City")
	f, err := NewGeoIPDB(confPath)
	if err != nil {
...
// TestGeoIPDBMissingConf ensures a missing conf file returns an error.
func TestGeoIPDBMissingConf(t *testing.T) {
	if _, err := NewGeoIPDB(filepath.Join(t.TempDir(), "nope.conf")); err == nil {
