...
// sources using ippolicy and geoip. When used as a handler, callers pass in
// pre-built IPPrefixSet and GeoIPDB values via the Blacklist and Geo fields.
type FormMailer struct {
	...
	Blacklist *ippolicy.IPPrefixSet
	...
	// Run-only state
	blocklistSet   *ippolicy.IPPrefixSet
	geoDB          *geoip.GeoIPDB
	server         *http.Server
...
//
// Data sources:
//   - Blocklist: ippolicy.IPPrefixSet loads the configured repository files.
//   - GeoIP: geoip.GeoIPDB downloads and atomically publishes City + ASN.
//
...
func (fm *FormMailer) Run(ctx context.Context) error {
	...
	// Blocklist is a repository-backed IPPrefixSet.
	var blocklistSet *ippolicy.IPPrefixSet
	if fm.BlocklistRepo != "" {
		var err error
		blocklistSet, err = ippolicy.NewIPPrefixSet(ctx, fm.BlocklistRepo, filepath.Join(fm.CacheDir, "bitwire-it"), fm.BlocklistPaths, fm.RefreshInterval)
		if err != nil {
	...
	// geoip.GeoIPDB); otherwise, if the archive dir is already populated,
	...
	// cache layer's job; freshness gating is httpcache's
	...
	var geoDB *geoip.GeoIPDB
	geoMode := "disabled"
	switch {
	case confExists:
		geoMode = "download"
		var ferr error
		geoDB, ferr = geoip.NewGeoIPDB(fm.GeoIPConfPath)
		if ferr != nil {
			log.Printf("geoip: %v", ferr)
		} else {
			geoDB.Dir = geoDir
			geoDB.BaseURL = fm.GeoIPBaseURL
			geoDB.FreshDays = fm.GeoIPFreshDays
		}
	case dirPopulated(geoDir):
		geoMode = "poll"
		geoDB = geoip.NewLocalGeoIPDB(geoDir)
	}

	// Keep the shared cache instances used by ServeHTTP in sync with Run.
	...
	if blocklistSet != nil {
		...
		t := time.Now()
		if _, err := blocklistSet.Load(fm.ctx, true); err != nil {
			fmt.Fprintln(os.Stderr)
			log.Printf("blocklist: %v", err)
		} else {
			sz := 0
			if v := blocklistSet.Current(); v != nil {
				sz = v.Size()
			}
	...
	if geoDB != nil {
		...
		t := time.Now()
		if _, err := geoDB.Load(fm.ctx, true); err != nil {
			fmt.Fprintln(os.Stderr)
	...
	// Sources are revalidated lazily by requests. There are no package-owned
	// ticker goroutines; fetchers provide their own timestamp/cache gates.
	...
	// Healthz endpoint.
	mux.HandleFunc("GET /healthz", fm.healthz(blocklistSet, geoDB))
...
// healthz returns a handler that reports cache load status.
func (fm *FormMailer) healthz(
	blocklist *ippolicy.IPPrefixSet,
	geo *geoip.GeoIPDB,
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		cohortStatus := func(v *ippolicy.IPPrefixSet) dsStatus {
		...
		if geo != nil {
			s := dsStatus{LoadedAt: geo.Status().LoadedAt}
			if v := geo.Current(); v != nil {
				s.Loaded = true
			}
			status["geoip"] = s
		}
...
func (fm *FormMailer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// A request is an opportunity to revalidate. Revalidation runs in the
	// background, so the request always uses the newest completed snapshot.
	if fm.blocklistSet != nil {
		go func() {
			if _, err := fm.blocklistSet.Load(fm.ctx, false); err != nil && !errors.Is(err, context.Canceled) {
	...
	if fm.geoDB != nil {
		go func() {
			if _, err := fm.geoDB.Load(fm.ctx, false); err != nil && !errors.Is(err, context.Canceled) {
	...
	var blacklist *ippolicy.IPPrefixSet
	var geo *geoip.GeoIPDB
	if fm.BlocklistRepo != "" {
		blacklist = fm.blocklistSet
	} else {
		blacklist = fm.Blacklist
	}
	if fm.BlocklistRepo != "" || fm.geoDB != nil {
		geo = fm.geoDB
	} else {
		geo = fm.Geo
	}
	...
	if blacklist != nil {
		if c := blacklist.Current(); c != nil && c.ContainsAddr(ip) {
