...
func TestMnemonicVectors(t *testing.T) {
	for i, vec := range fixtures {
		...
		t.Run(fmt.Sprintf("vector-%d", i+1), func(t *testing.T) {
			...
			// 1. entropy → mnemonic
			gotMnemonic := base2048.EncodeToString(entropy)
			if gotMnemonic != expectedMnemonic {
				t.Errorf("mnemonic mismatch\nexpected: %q\ngot:      %q", expectedMnemonic, gotMnemonic)
			}

			// 3. mnemonic + passphrase "TREZOR" → seed
			seed, err := SeedFrom(gotMnemonic, "TREZOR")
			if err != nil {
				t.Fatalf("SeedFrom failed: %v", err)
			}

			gotSeedHex := hex.EncodeToString(seed)
			if gotSeedHex != expectedSeedHex {
				t.Errorf("seed mismatch\nexpected: %s\ngot:      %s", expectedSeedHex, gotSeedHex)
			}
		})
	}
}
...
func TestRoundTrip(t *testing.T) {
	...
	for _, bits := range strengths {
		t.Run(fmt.Sprintf("%d-bits", bits), func(t *testing.T) {
			mnemonic, err := Generate(bits)
			if err != nil {
				t.Fatal(err)
			}

			entropy, err := base2048.DecodeString(mnemonic)
			if err != nil {
				t.Fatalf("Decode failed: %v\nmnemonic: %s", err, mnemonic)
			}

			roundMnemonic := base2048.EncodeToString(entropy)

			if roundMnemonic != mnemonic {
				t.Errorf("round-trip failed\noriginal:  %s\nregenerated: %s", mnemonic, roundMnemonic)
			}
		})
	}
}
