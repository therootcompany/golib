...
import (
	...
	"github.com/therootcompany/golib/ulid/v2"
)
...
func TestCaseInsensitivity(t *testing.T) {
	...
	err := quick.CheckEqual(upper, lower, &quick.Config{
		MaxCount:      10_000,
		MaxCountScale: 0,   // use MaxCount
		Rand:          nil, // psuedorandom is ok
		Values:        nil, // uses reflect to discover it should fill [16]byte
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCaseIL(t *testing.T) {
	t.Parallel()

	a, err := ulid.Parse("0LLLLLLLLLLLLLLLLLLLLLLLLL")
	if err != nil {
		t.Fatal(err)
	}
	b, err := ulid.Parse("01111111111111111111111111")
	if err != nil {
		t.Fatal(err)
	}
	if a != b {
		t.Fatal(fmt.Errorf("1 and L should be treated as equal"))
	}
}

func TestCase0O(t *testing.T) {
	t.Parallel()

	c, err := ulid.Parse("00000000000000000000000000")
	if err != nil {
		t.Fatal(err)
	}
	d, err := ulid.Parse("0OOOOOOOOOOOOOOOOOOOOOOOOO")
	if err != nil {
		t.Fatal(err)
	}
	if c != d {
		t.Fatal(fmt.Errorf("0 and O should be treated as equal"))
	}
}

func TestCaseU(t *testing.T) {
	t.Parallel()

	_, err := ulid.Parse("0UUUUUUUUUUUUUUUUUUUUUUUUU")
	if err == nil {
		t.Fatal(fmt.Errorf("U should not be treated as valid"))
	}
}
...
func TestParseRobustness(t *testing.T) {
	...
	for _, tc := range cases {
		if _, err := ulid.DecodeRaw(string(tc)); err != nil {
			t.Error(err)
		}
	}
	...
	prop := func(s [26]byte) (ok bool) {
		...
		var err error
		if _, err = ulid.DecodeRaw(string(s[:])); err != nil {
			t.Error(err)
		}
